# VK Console Bot — логика PromptMaster (архивная копия)

> Это **независимый снимок** консольной программы на момент выделения VK-бота в `vk_prompt_bot/`.
> Актуальная разработка ведётся в **`vk_console_bot/`** в корне репозитория; этот каталог сохранён, чтобы можно было вернуться к исходному состоянию файлов или сравнить версии.

Модуль с промптами для OpenAI-совместимого API и консольным REPL. VK-бот подключает движок из **`vk_console_bot/`**, не из этой папки.

Пути ниже из оригинальной документации: для работы из **этого** каталога замените `vk_console_bot` на **`vk_console_bot_original`** в командах `cd`.
